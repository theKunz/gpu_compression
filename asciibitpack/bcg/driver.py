...
def run_single_test(library, test):
    original_file = test.file
    compressed_file = test.file + '.bcg'
    decompressed_file = test.file + '.bcg.out'
    original_size = os.path.getsize(original_file)*0.000001

    #Compresssion
    cmdline = './' + library.executable + ' ' + library.compression_switch + ' ' + original_file
    ...
    #Decompression
    cmdline = './' + library.executable + ' ' + library.decompression_switch + ' ' + compressed_file
    start_time = time.time()
    proc = subprocess.Popen(cmdline , stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE, shell=True)
    proc.wait()
